...
declare _mydata cursor FOR SELECT indexname, indexdef FROM pg_indexes WHERE tablename = _tablename;
        _rec record;
    open _mydata;
    for _rec in _mydata loop
        raise notice '%, %', _rec.indexname, _rec.indexdef;
    end loop;
    close _mydata;
$$ language plpgsql;
